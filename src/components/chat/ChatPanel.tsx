...
export function ChatPanel() {
  ...
  const mentionableUsers = useMemo(() => {
    ...
    return [...users]
      .sort(compareUserPresencePriority)
      .filter((candidate) => {
  ...
  const mentionMatchCount = useMemo(
    () =>
      parsedMessages.filter(
        (message) => !message.system && Boolean(message.mentionMeta?.mentionsCurrentUser),
      ).length,
    [parsedMessages],
  );
  const visibleMessages = useMemo(
    () =>
      parsedMessages.filter((message) => {
        ...
        return !message.system && Boolean(message.mentionMeta?.mentionsCurrentUser);
      }),
    [parsedMessages, showMentionedOnly],
  );
  ...
  const isMentionMenuOpen = Boolean(mentionDraft && mentionSuggestions.length > 0);
  ...
  const handleInputKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === "Enter" || event.key === "Tab") {
      event.preventDefault();
      const nextIndex = Math.min(activeMentionIndex, mentionSuggestions.length - 1);
      ...
      <div className="flex items-start justify-between gap-2 border-b border-[rgba(255,255,255,0.06)] px-3 py-2">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-[var(--text-secondary)]">
            Conversa em tempo real com badges de cargo e menções.
          </p>
        </div>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    <span className="inline-flex items-center gap-1">
                      <RoomRoleIcon className={`h-3 w-3 shrink-0 ${roomRole.iconClassName}`} />
                    ...
                    <span className={`truncate text-[11px] font-semibold ${roomRole.nameClassName}`}>
                      {item.username}
                    </span>
                  ...
                  <div className="mt-1 space-y-1">
                    {item.messages.map((message) => (
                      <div
                        key={message.id}
                        className={`max-w-full rounded-2xl border px-2.5 py-1.5 ${
                          message.mentionMeta.mentionsCurrentUser
                            ? "border-[rgba(255,196,102,0.24)] bg-[linear-gradient(135deg,rgba(73,45,17,0.44),rgba(31,23,16,0.28))] shadow-[0_12px_24px_rgba(255,196,102,0.08)]"
                            : message.mentionMeta.hasMentions
                              ? "border-[rgba(94,185,255,0.14)] bg-[rgba(18,34,51,0.28)]"
                              : "border-[rgba(255,255,255,0.04)] bg-[rgba(255,255,255,0.02)]"
                        }`}
                      >
                        <p className="break-words text-[12px] leading-[1.45] text-[var(--text-secondary)]">
                          {message.mentionMeta.segments.map((segment, segmentIndex) => {
                            if (segment.type === "text") {
                              return <span key={`${message.id}-text-${segmentIndex}`}>{segment.text}</span>;
                            }

                            return (
                              <span
                                key={`${message.id}-mention-${segmentIndex}`}
                                className={`mx-[1px] inline-flex items-center rounded-md border px-1 py-[1px] align-[0.02em] font-semibold ${
                                  segment.isCurrentUser
                                    ? "border-[rgba(255,196,102,0.3)] bg-[rgba(88,54,16,0.48)] text-[#ffd289]"
                                    : "border-[rgba(94,185,255,0.2)] bg-[rgba(23,54,86,0.44)] text-[#a7dbff]"
                                }`}
                              >
                                {segment.text}
                              </span>
              ...
              >
                ...
                <div className="max-h-56 overflow-y-auto p-1.5">
                  {mentionSuggestions.map((candidate, index) => {
                    ...
                    const platformRole = getPlatformRoleMeta(candidate.platformRole);
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <p className={`truncate text-[12px] font-semibold ${roomRole.nameClassName}`}>
                            {candidate.username}
                          </p>
                          <div className="mt-0.5 flex items-center gap-1.5 text-[10px] text-[var(--text-muted)]">
                            <span className="truncate">{roomRole.label}</span>
                            {normalizePlatformRole(candidate.platformRole) !== "none" && (
                              <span className="inline-flex items-center gap-1 truncate">
                                <PlatformIcon className={`h-3 w-3 ${platformRole.iconClassName}`} />
                                {platformRole.label}
                              </span>
                            )}
                          </div>
