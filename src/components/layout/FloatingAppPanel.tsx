...
import { LayoutGrid, ListMusic, ShoppingBag, User2, Users, X } from "lucide-react";
...
export function FloatingAppPanel() {
  ...
  const profileId = panel?.view === "profile" ? panel.profileId ?? user?.id ?? null : null;

  const content =
    panel?.view === "rooms" ? <RoomsPage /> :
    panel?.view === "playlists" ? <PlaylistsPage /> :
    panel?.view === "friends" ? <FriendsPage /> :
    panel?.view === "shop" ? <ShopPage /> :
    panel?.view === "profile" && profileId ? <ProfilePage profileId={profileId} /> :
    null;
